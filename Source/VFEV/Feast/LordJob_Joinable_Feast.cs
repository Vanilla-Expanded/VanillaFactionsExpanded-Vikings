...
namespace VFEV
{
    public class LordJob_Joinable_Feast : LordJob_Joinable_Gathering
    {
        ...
        public new int DurationTicks => durationTicks;
        ...
        public override StateGraph CreateGraph()
        {
            ...
            transition.AddTrigger(new Trigger_PawnLost(PawnLostCondition.Killed, organizer));
            ...
            for (int i = 0; i < ownedPawns.Count; i++)
            {
                ...
                if (lordToilData_Party.presentForTicks.TryGetValue(pawn, out int value) && value > 0)
                {
                    if (ownedPawns[i].needs.mood != null)
                    {
                        ...
                        float moodPowerFactor = Mathf.Min(value / (float)durationTicks + num, 1f);
